...
func main() {
	...
	agentHandler := rest.NewAgentHandler(agentSvc, adapters.NewResumeParser())
	jobHandler := rest.NewJobHandler(jobRepo, agentRepo)
	matchHandler := rest.NewMatchHandler(matchSvc)
