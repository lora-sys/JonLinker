...
export default function ConversationPage({ params }: PageProps) {
  ...
  const determineAgentRole = useCallback((msg: { role: string; id?: string }): AgentRole => {
    if (msg.role === 'user') return 'seeker';
    if (msg.role === 'system') return 'system';
    return 'recruiter';
  }, []);

  const getMessageContent = useCallback((msg: { parts?: Array<{ type: string; text?: string }>; role: string }): string => {
    if (msg.parts) {
      return msg.parts
